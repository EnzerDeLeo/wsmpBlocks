...
public abstract class BlockBrickExtraSlab extends BlockSlab
{
	public static final PropertyEnum VARIANT = PropertyEnum.create("variant", BlockBrickExtra.Type.class);
	public static final String NAME = "brickextraslab";

	public BlockBrickExtraSlab(Material materialIn)
	{
		super(materialIn);
		...
		this.setDefaultState(blockState.withProperty(VARIANT, BlockBrickExtra.Type.BRIGHTRED));
	...
	{
		return NAME + "." + BlockBrickExtra.Type.byMetadata(metadata).getUnlocalizedName();
	}
	...
	{
		return BlockBrickExtra.Type.byMetadata(stack.getMetadata() & 7);
	}
	...
	{
		IBlockState iblockstate = this.getDefaultState().withProperty(VARIANT, BlockBrickExtra.Type.byMetadata(meta & 7));
	...
	{
		byte b0 = 0;
		int i = b0 | ((BlockBrickExtra.Type) state.getValue(VARIANT)).getMetadata();
	...
	{
		return ((BlockBrickExtra.Type) state.getValue(VARIANT)).getMetadata();
	}
